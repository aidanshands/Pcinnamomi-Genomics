...
ad <- extract.gt(vcf, element = 'AD')
hist(ad2[,"Pc2109"], breaks = seq(0,1,by=0.02), col = "#1f78b4", xaxt="n")
hist(ad1[,"Pc2109"], breaks = seq(0,1,by=0.02), col = "#a6cee3", add = TRUE)
...
tmp3 <- allele1[,"Pc2109"]
tmp3 <- tmp3[tmp3 <= 250]
hist(tmp3, breaks=seq(0,250,by=1), col="#808080", main = "Pc2109")
sums <- apply(allele1, MARGIN=2, quantile, probs=c(0.15, 0.95), na.rm=TRUE)
sums[,"Pc2109"]
abline(v=sums[,"Pc2109"], col=2, lwd=2)
